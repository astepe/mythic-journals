#!/usr/bin/env bash
# Serve a realm's Jekyll site locally at http://localhost:4000
# Auto-detects the realm name from the current directory.
...
if command -v docker >/dev/null 2>&1; then
  if docker info >/dev/null 2>&1; then
    ...
    echo "Open http://localhost:4000/$REALM/ in your browser"
    ...
    docker run --rm $TTY_FLAG -p 4000:4000 \
      -v "$(pwd):/var/jekyll" \
      -v "$REALM-bundle-arm64:/usr/local/bundle" \
