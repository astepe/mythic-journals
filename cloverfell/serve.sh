#!/usr/bin/env bash
# Serve the Cloverfell Jekyll site locally at http://localhost:4000
...
if command -v docker >/dev/null 2>&1; then
  if docker info >/dev/null 2>&1; then
    ...
    echo "Open http://localhost:4000/cloverfell/ in your browser"
    ...
    docker run --rm $TTY_FLAG -p 4000:4000 \
      -v "$(pwd):/var/jekyll" \
      -v cloverfell-bundle-arm64:/usr/local/bundle \
